...
test("max-range prop when type is range and showConfirm is false", async () => {
  ...
  const days = wrapper.findAll(".vm-calendar-day");
...
test("show-title prop", () => {
  ...
  expect(wrapper.contains(".vm-calendar-header-title")).toBe(true);
});

test("show-title prop false", () => {
  const wrapper = mount(Calendar, {
    propsData: {
      value: true,
      showTitle: false
  ...
  expect(wrapper.contains(".vm-calendar-header-title")).toBe(false);
});
...
test("show-subtitle prop", () => {
  ...
  expect(wrapper.contains(".vm-calendar-header-subtitle")).toBeTruthy();
  wrapper.setProps({ showSubtitle: false });
  expect(wrapper.contains(".vm-calendar-header-subtitle")).toBeFalsy();
});
...
test("hide close icon when there is no title", () => {
  ...
  expect(wrapper.contains(".vm-popup-close-icon")).toBeTruthy();

  wrapper.setProps({
    showTitle: false,
    showSubtitle: false
  });
  expect(wrapper.contains(".vm-popup-close-icon")).toBeFalsy();
});
...
test("min-date after current time", () => {
  ...
  wrapper.find(".vm-calendar-confirm").trigger("click");
...
test("first day of week", async () => {
  ...
  expect(wrapper.find(".vm-calendar-weekdays").text()[0]).toEqual("二");

  const day = wrapper.find(".vm-calendar-month:first-of-type .vm-calendar-day");
