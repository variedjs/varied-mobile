<template>
  <div v-if="isShow" :class="b()" @touchmove.prevent @click="hide">
    <div :class="b('content')" @click.stop>
      <p :class="b('content-title')">
        请选择
        <icon
      ...
      <ul :class="b('content-tab', { landscape: landscape })">
        <li
      ...
      <ul :class="b('content-data')">
        ...
        >
          <span :class="{ selected: item.name === tabList[tabIndex].name }">
            {{ item.name }}
          </span>
        </li>
      </ul>
  ...
  methods: {
    ...
    get(level, id, change) {
      ...
      if (data && data.length > 0) {
        ...
        self.showData = showData.sort(function compareFunction(param1, param2) {
